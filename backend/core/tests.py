...
class BusinessRulesTests(TestCase):
    ...
    def test_ai_catalog_request_ignores_stale_story_context(self):
        self.item.status = "available"
        self.item.name_uz = "Gortenziya Mix"
        self.item.arrangement_type = "basket"
        self.item.price = 1500000
        self.item.save(update_fields=["status", "name_uz", "arrangement_type", "price", "updated_at"])
        CatalogItem.objects.create(branch=self.branch, name_uz="Pion buketi", name_ru="Pion bouquet", arrangement_type="bouquet", price=800000, status="available")
        post = SocialPost.objects.create(branch=self.branch, post_type="story", media_id="story-gortenziya", title_uz="Gortenziya Mix", title_ru="Hydrangea Mix", price=1500000, is_active=True)
        self.item.social_post = post
        self.item.save(update_fields=["social_post", "updated_at"])
        customer = Customer.objects.create(branch=self.branch, instagram_user_id="ig-catalog-stale")
        conversation = Conversation.objects.create(customer=customer, branch=self.branch, social_post=post)
        conversation.messages.create(sender="customer", text="katalogni korsat")
        payload = {
            "reply": "Gortenziya Mix savat\nNarxi 1 500 000 so'm\nSizga qachonga kerak edi?",
            "detected_language": "uz",
            "customer_name": None,
            "phone": None,
            "lead_ready": False,
            ...
            "handoff": False,
            "catalog_items": [{"catalog_name": "Gortenziya Mix", "quantity": 1}],
            "stock_items": [],
        }
        from unittest.mock import patch
        with patch("core.services.ai_reply", return_value=payload):
            reply = create_ai_reply_for_conversation(conversation)
        self.assertIn("Katalogdagi tayyor gullarimiz", reply.text)
        self.assertIn("Gortenziya Mix savat 1 500 000 so'm", reply.text)
        self.assertIn("Pion buketi buket 800 000 so'm", reply.text)
        self.assertNotIn("Katalogimizda hozir faqat", reply.text)

    def test_ai_bouquet_catalog_request_filters_bouquets(self):
        self.item.status = "available"
        self.item.name_uz = "Gortenziya Mix"
        self.item.arrangement_type = "basket"
        self.item.price = 1500000
        self.item.save(update_fields=["status", "name_uz", "arrangement_type", "price", "updated_at"])
        CatalogItem.objects.create(branch=self.branch, name_uz="Pion buketi", name_ru="Pion bouquet", arrangement_type="bouquet", price=800000, status="available")
        customer = Customer.objects.create(branch=self.branch, instagram_user_id="ig-bouquet-filter")
        conversation = Conversation.objects.create(customer=customer, branch=self.branch)
        conversation.messages.create(sender="customer", text="buketlar yoqmi katalogda")
        payload = {
            "reply": "Gortenziya Mix savat\nNarxi 1 500 000 so'm\nSizga qachonga kerak edi?",
            "detected_language": "uz",
            "customer_name": None,
        ...
        from unittest.mock import patch
        with patch("core.services.ai_reply", return_value=payload), patch("core.services.instagram_send_image", return_value={"ok": True}):
            reply = create_ai_reply_for_conversation(conversation)
        self.assertIn("Katalogimizda hozir faqat Pion buketi buket bor ekan", reply.text)
        self.assertIn("Narxi 800 000 so'm", reply.text)
        self.assertNotIn("Gortenziya Mix", reply.text)
