...
O'zbek kirillda alohida ehtiyot bo'l. To'g'ri harflar:
  ҳ  emas  х   masalan ҳақи, ҳозир, шаҳар
  қ  emas  ќ   masalan қайси, нархқ emas нарх
  ў  emas  у   masalan сўм, кўчаси, дўкон
  ғ  emas  г   masalan боғ, тўғри
  ж  emas  ҷ yoki җ   masalan менежер, жами
  е  emas  йе   masalan етказиб бериш
Kirill javobda lotin harf ishlatma. "Florist" emas "Флорист", "Atirgul" emas "Атиргул".
"кунгиз ҳаёри" xato, to'g'risi "кунингиз хайрли".
...
Boshqa tez-tez uchraydigan shakllar:
  ...
  yasab bering, qilib bering, qso, yasatsam, yig'dirsam    → buket yoki savat yasatish
...
Keyin florist haqi. Undan keyin BO'SH QATOR. Keyin jami va operator izohi.
...
"Aniq narxni operatorlarimiz sizga aytishadi" qatori faqat gul yasatish, ya'ni custom
...
def set_working_hours(apps, schema_editor):
    BusinessSettings = apps.get_model("core", "BusinessSettings")
    for row in BusinessSettings.objects.all():
        hours = row.working_hours or {}
        hours["uz"] = "24/7, kunu tun ochiq"
        hours["ru"] = "24/7, круглосуточно"
        hours.setdefault("timezone", "Asia/Tashkent")
...
class Migration(migrations.Migration):
    dependencies = [
        ("core", "0077_ai_prompt_discount_variation"),
    ]

    operations = [
        migrations.RunPython(set_sales_prompt, migrations.RunPython.noop),
        migrations.RunPython(set_working_hours, migrations.RunPython.noop),
    ]
